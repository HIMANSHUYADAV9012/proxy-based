...
def format_error_message(username: str, proxy_url: str, attempt: int, error: str, status_code: int = None):
    if status_code:
        return (
            f"❌ Error | {username}\n"
            f"🔁 Try: {attempt} | 🌐 Proxy: {proxy_url}\n"
            f"📡 {status_code} | {error}"
        )
    else:
        return (
            f"⚠️ Exception | {username}\n"
            f"🔁 Try: {attempt} | 🌐 Proxy: {proxy_url}\n"
            f"❗ {error}"
        )
...
async def scrape_user(username: str, max_retries: int = 2):
    ...
    for attempt in range(max_retries):
        ...
        proxy_url = str(client._proxies.get("http"))  # just for logging
        ...
        try:
            ...
            if result.status_code == 200:
                ...
                await notify_telegram(f"✅ Success | {username} | Proxy OK")
                return user_data
    ...
    await handle_error(502, "All proxies failed", f"🚨 All proxies failed | {username}")
...
async def proxy_image(request: Request, url: str, max_retries: int = 2):
    for attempt in range(max_retries):
        client = get_random_client()
        proxy_url = str(client._proxies.get("http"))
        ...
        try:
            ...
            if resp.status_code == 200:
                STATS["proxy_success"] += 1
                await notify_telegram(f"✅ Image fetched | Proxy OK")
